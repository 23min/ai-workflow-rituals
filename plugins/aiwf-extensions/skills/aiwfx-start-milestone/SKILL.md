...
description: Sets up and begins an aiwf milestone — preflight checks, branch setup, status promotion to in_progress, then iterative TDD via wf-tdd-cycle. Use when the user says "start milestone M-NNN" or "implement M-NNN" and a draft milestone spec exists. Commits and pushes require explicit human approval.
---
...
- Invoke `wf-tdd-cycle` (red → green → refactor → done). When the milestone is `tdd: required`, `wf-tdd-cycle` drives `aiwf promote M-NNN/AC-<N> --phase <p>` at each phase transition; the timeline shows up in `aiwf history M-NNN/AC-<N>` automatically.
...
- Append a Work log entry to the milestone spec's `## Work log` section: `### AC-<N> — <short title>` followed by `<one-line outcome> · commit <SHA> · tests <N/M>`. Don't duplicate the phase timeline — `aiwf history M-NNN/AC-<N>` is the authoritative record.
...
If a piece of work surfaces that's deferred, open a gap (`aiwf add gap --title "..." --discovered-in M-NNN`) and mirror the resulting `G-NNN` id under the spec's `## Deferrals` section.
...
- Run `aiwf check` (or `aiwf show M-NNN`); confirm zero error-severity findings on the milestone. The `acs-tdd-audit`, `milestone-done-incomplete-acs`, and `acs-shape` codes are the AC-related ones to watch for.
...
- *Hand-editing `acs[]` in frontmatter.* Use `aiwf add ac` / `aiwf promote M-NNN/AC-<N>` / `aiwf rename M-NNN/AC-<N>` instead — the verbs preserve position-stability and the body-coherence pairing.
